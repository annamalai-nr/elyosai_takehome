# LLM Prompting: Best Practices for General Usage

This guide shares principles and structural patterns that work well across modern large language models. These aren't meant to be treated as the gold standard, but rather as a starting point to help you refine your own strategies. Take what works, tweak what doesn't, and keep iterating.

## Core Principles

Modern LLMs favor directness over persuasion and logic over verbosity. To maximize performance, adhere to these core principles:

- **Precise Instructions**: Be concise in your input prompts. LLMs respond best to direct, clear instructions. State your goal clearly without fluff.
...
- **Output Verbosity**: By default, many LLMs prefer providing direct, efficient answers. If you require a more conversational or "chatty" persona, you must explicitly ask for it.
- **Multimodal Coherence**: For multimodal models, text, images, audio, or video should all be treated as equal-class inputs. Instructions should reference specific modalities clearly to ensure the model synthesizes across them rather than analyzing them in isolation.
...
<instructions>
1. **Plan**: Analyze the task and create a step-by-step plan with distinct sub-tasks.
2. **Execute**: Carry out the plan. If using tools, reflect before every call. Track your progress in a TODO List using [ ] for pending, [x] for complete.
