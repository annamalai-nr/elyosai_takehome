...
response:
  - saves the full HTTP response (status + headers + body) to a .raw file
    in this folder (initial_api_invocations/)
...
The script reads ELYOS_API_KEY from .env (in the PROJECT ROOT, one level
up from this script) via python-dotenv. You can also pass the key explicitly:
    python initial_api_invocations/probe_injection.py <your-key>

Output files (saved next to this script, in initial_api_invocations/):
...
SCRIPT_DIR = Path(__file__).parent           # initial_api_invocations/
PROJECT_ROOT = SCRIPT_DIR.parent             # project root (where .env lives)
...
def main():
    ...
    # python-dotenv silently no-ops if the file is missing, so passing-key-
    # as-arg still works.
    ...
    if not api_key:
        ...
        print("  python initial_api_invocations/probe_injection.py", file=sys.stderr)
        print("OR:", file=sys.stderr)
        print("  python initial_api_invocations/probe_injection.py <your-key>", file=sys.stderr)
        sys.exit(2)
